...
public interface TaskRepository extends JpaRepository<Tasks, UUID> {
    ...
    void deleteByTaskListIdAndId(UUID taskList_id, UUID id);
    List<Tasks> findByPriorityAndStatusAndDueDateBetween(
            TaskPriority priority,TaskStatus status, LocalDateTime start, LocalDateTime end);
}
